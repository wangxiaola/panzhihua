...
@interface ZKSureOrderViewController ()
...
@implementation ZKSureOrderViewController
...
- (void)setupData
{
    ...
    self.priceLabel.text = [NSString stringWithFormat:@"%@元", self.order.price];
    self.dateLabel.text = self.order.qixian.length ? [self.order.qixian stringByReplacingOccurrencesOfString:@"," withString:@"到"] : @"无期限";
