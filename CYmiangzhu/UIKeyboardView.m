...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		UIBarButtonItem *previousBarItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"上栏", @"")
																			style:UIBarButtonItemStylePlain
																		   target:self
																		   action:@selector(toolbarButtonTap:)];
		previousBarItem.tag=1;
		
		UIBarButtonItem *nextBarItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"下栏", @"")
																		style:UIBarButtonItemStylePlain
																	   target:self
																	   action:@selector(toolbarButtonTap:)];
		nextBarItem.tag=2;
