...
#define frameWeekLabelHeight self.frame.size.width/13
#define dayWHRatio 1.0
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        NSArray * shortWeekdaySymbols = @[@"日",@"一",@"二",@"三",@"四",@"五",@"六"];
...
- (void)drawRect:(CGRect)rect
{
    ...
    UIButton * buttonPrev          = [[UIButton alloc] initWithFrame:CGRectMake(_originX, frameWeekLabelHeight-5, _dayWidth, _dayWidth)];
    ...
    UIButton * buttonNext          = [[UIButton alloc] initWithFrame:CGRectMake(self.bounds.size.width - _dayWidth - _originX, frameWeekLabelHeight-5, _dayWidth, _dayWidth)];
    ...
    if (!_hideMonthLabel)
    {
        UILabel *titleText              = [[UILabel alloc]initWithFrame:CGRectMake(0,frameWeekLabelHeight+5, self.bounds.size.width, frameWeekLabelHeight)];
        titleText.textAlignment         = NSTextAlignmentCenter;
        titleText.text                  = dateString;
        titleText.font                  = _titleFont;
        titleText.textColor             = _monthAndDayTextColor;
        [self addSubview:titleText];
        self.monthLabel = titleText;
    }
    ...
    // Day labels
    UIView *weekLabelBg = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, frameWeekLabelHeight)];
    weekLabelBg.backgroundColor = [UIColor colorWithRed:249/255.0 green:249/255.0 blue:249/255.0 alpha:1];
    [self addSubview:weekLabelBg];
    __block CGRect frameWeekLabel = CGRectMake(0, 0, _dayWidth, frameWeekLabelHeight);
    [_weekDayNames  enumerateObjectsUsingBlock:^(NSString * dayOfWeekString, NSUInteger idx, BOOL *stop)
     {
         ...
         [weekLabelBg addSubview:weekNameLabel];
     }];
    ...
    for (NSInteger i= 0; i<monthLength; i++)
    {
        ...
        UIButton *button    = [self dayButtonWithFrame:CGRectMake(_originX+offsetX, CGRectGetMaxY(self.monthLabel.frame)+offsetY, _dayWidth, _dayWidth*dayWHRatio)];
