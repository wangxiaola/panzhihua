...
-(void)didSelect{
  ...
  [self hideButtonClick];
    ...
    if ([self.delegate respondsToSelector:@selector(Pconfirm:tp:)]) {
        [self.delegate Pconfirm:secel tp:tg];
    }
}
...
-(id)initNames:(NSArray*)type sect:(NSInteger)p reminder:(NSString*)r tp:(NSInteger)j;
{
    ...
    if (self) {
        ...
        for (int i=0; i<type.count; i++) {
            ...
            NSArray *tits =[type[i] componentsSeparatedByString:@"#"];
            name.text =tits[1];
