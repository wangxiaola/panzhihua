...
- (void)viewDidLoad {
    ...
    [[BaiduMobStat defaultStat] logEvent:@"btn_update_password" eventLabel:@"修改密码 "];
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if(timeout<=0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [weakSelf.random setTitleColor:CYBColorGreen forState:0];
...
-(void)loginClick
{
    ...
    para[@"interfaceId"] = @23;
    para[@"method"] = @"getMember";
    para[@"account"] = phoneField.text;
    ...
    [ZKHttp post:ZKPostUrl params:para success:^(id responseObj) {
        ...
        if ([responseObj[@"errmsg"] isEqualToString:@"SUCCESS"]) {
            ...
            para1[@"interfaceId"] = @24;
            para1[@"method"] = @"modifyPwd";
            para1[@"id"] = responseObj[@"root"][@"member"][@"id"];
            para1[@"password"] =paswordField.text;
            
            [ZKHttp post:ZKPostUrl params:para1 success:^(id responseObj) {
           ...
           [SVProgressHUD showErrorWithStatus:@"修改密码失败" duration:1];
...
-(void)loginPost
{
    ...
    [dic setObject:phoneField.text forKey:@"phone"];
    [dic setObject:paswordField.text  forKey:@"newcode"];
    [dic setValue:identifyingField.text forKey:@"code"];
    
    [ZKHttp Post:@"192.168.0.142:8081/travel/interfaceFilter.in?cid=112" params:dic success:^(id responseObj) {
        
        [_activityView stopAnimating];
        NSLog(@"---%@",responseObj);
        if ([[responseObj valueForKey:@"success"] isEqualToString:@"true"]) {
            [ZKUtil MyboolForKey:@"information" setBool:YES];
            [ZKUtil MyValue:paswordField.text MKey:@"myPasword"];
            [ZKUtil MyValue:phoneField.text MKey:@"myPhone"];
            [ZKUtil MyValue:[responseObj valueForKey:@"data"] MKey:@"myId"];
            
            ZKMoreReminderView *po =[[ZKMoreReminderView alloc]initTs:@"温馨提示" MarkedWords:@"密码已找回,返回登录页面吗？"];
            [po show];
            [po sectec:^(int pgx) {
            ...
            ZKMoreReminderView *po =[[ZKMoreReminderView alloc]initTs:@"温馨提示" MarkedWords:@"亲,该账号已注册"];
        ...
        [_activityView stopAnimating];
        [self.view makeToast:@"网络出错的，请检查网络连接！" duration:1 position:nil];
    }];
}
